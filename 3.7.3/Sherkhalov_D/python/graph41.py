...
y = kappa**2 - alpha**2
dy = 2*kappa*dk + 2*alpha*da
x = 4*(np.pi)**2 * nu**2
dx = 8*(np.pi)**2 * nu *dnu

plt.xlabel(r'$x_1,\, s^{-2}$, ', fontsize=14)
plt.ylabel(r'$y_1,\, cm^{-1}$', fontsize=14)
plt.title(r'$y_1(x_1)$', fontsize=14)
plt.grid(True)
...
plt.plot(x, yfit, color="firebrick", label=r'МНК $y_1(x_1)$')
