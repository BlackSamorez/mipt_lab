...
# --- 1. Чтение данных из G.xlsx ---
try:
    df = pd.read_excel('G.xlsx', sheet_name='Sheet1', header=None)
    if len(df) < 2:
        print("Ошибка: Не хватает данных в G.xlsx.")
        exit()

    T_series = df.iloc[0, :].dropna()
    delta_nu_series = df.iloc[1, :].dropna()

    T_data = T_series[1:].astype(float).to_numpy()
    delta_nu_data = delta_nu_series[1:].astype(float).to_numpy()

    print("Данные успешно загружены из G.xlsx:")
    print(f"T (мкс): {T_data}")
    print(f"delta_nu (кГц): {delta_nu_data}")

except FileNotFoundError:
    print("Ошибка: Файл 'G.xlsx' не найден.")
    exit()
except Exception as e:
    print(f"Ошибка при чтении файла G.xlsx: {e}")
    import traceback
    traceback.print_exc()
    exit()

# --- 2. Подготовка данных и погрешностей ---
inv_T_data = 1.0 / T_data  # 1/T (в 1/мкс)
delta_nu_err = np.full_like(delta_nu_data, 0.001)  # Погрешность \delta \nu: 0.001 кГц
T_err_constant = 50.0  # мкс, постоянная погрешность T
inv_T_err = (1.0 / (T_data**2)) * T_err_constant  # Погрешность 1/T

print(f"\nВычисленные значения и погрешности:")
print(f"1/T (1/мкс): {inv_T_data}")
print(f"Погрешность 1/T (1/мкс): {inv_T_err}")
...
# Оценка R^2 для регрессии через 0
...
plt.errorbar(x_data, y_data, xerr=inv_T_err, yerr=delta_nu_err, fmt='o',
             ecolor='red', markersize=6, label='Экспериментальные данные')

# Оформление графика
plt.xlabel(r'$1/T$, 1/мкс')
plt.ylabel(r'$\delta\nu$, кГц')
plt.title('Зависимость расстояния между соседними гармониками от периода')
plt.legend()
plt.grid(True, alpha=0.5)
plt.xlim(0.0, x_max_plot)
# Для y_min_plot используем 0 или немного меньше минимума данных
y_min_plot = max(0.0, min(y_data) - 0.1 * (max(y_data) - min(y_data)))
plt.ylim(y_min_plot, max(y_data) * 1.2)
plt.tight_layout()

# Сохранение графика
output_filename = 'delta_nu_vs_inv_T_plot_G.png'
