...
def make_polyfit(file, title, xlabel, ylabel, x, y, dx, dy, degree):
    ...
    def func(x):
        return (sum([popt[degree-i]*x[0]**i for i in range(degree+1)]), sum([(x[1]*i*popt[degree-i]*(x[0]**(i-1))) for i in range(1, degree+1)]))
...
def make_curve_approx(file, title, xlabel, ylabel, x, y, dx, dy, func, p0):
    ...
    def res_func(x):
        return (func_(x[0]), derivative(func_, x[0]).df * x[1])

    x_lin = np.linspace(min(x), max(x), 1000)
    plt.plot(x_lin, [res_func((i, 0))[0] for i in x_lin], "b", label = "Аппроксимация")
    ...
    return res_func, [(i, j) for i, j in zip(popt, dpopt)]
